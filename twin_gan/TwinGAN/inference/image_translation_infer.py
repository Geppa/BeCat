...
tf.flags.DEFINE_string("model_path", "./128", "Path containing a checkpoint.")
tf.flags.DEFINE_integer('image_hw', 128, 'height and width of the input image.')
tf.flags.DEFINE_string('input_tensor_name', "sources_ph", 'Optional input tensor name. e.g. sources_ph.')
tf.flags.DEFINE_string('output_tensor_name', "custom_generated_t_style_source:0", 'e.g. custom_generated_t_style_source:0')
...
def main(_):
  ...
  if isinstance(outputs, list):
    ...
    for i, output in enumerate(outputs):
      ...
      '''
      path='/content/demo_dir'
      os.mkdir(path)
      path='/content/demo_dir/demo_class'
      os.mkdir(path)
      '''
      path='/content/demo_dir/demo_class/demo.png'
      util_io.imsave(path,output)
  else:
    ...
    '''
    path='/content/demo_dir/demo_class/demo.png'
    util_io.imsave(path,outputs)
    

if __name__ == '__main__':
  tf.flags.DEFINE_string('input_image_path', "./demo/inference_input/cropped",
  ...
  tf.flags.DEFINE_string('output_image_path', "./demo/inference_output/",
