...
tf.compat.v1.flags.DEFINE_string("model_path", "/content/drive/MyDrive/Colab/twin_gan/TwinGAN/128", "Path containing a checkpoint.")
tf.compat.v1.flags.DEFINE_integer('image_hw', 256, 'height and width of the input image.')
tf.compat.v1.flags.DEFINE_string('input_tensor_name', "sources_ph", 'Optional input tensor name. e.g. sources_ph.')
tf.compat.v1.flags.DEFINE_string('output_tensor_name', "custom_generated_t_style_source:0", 'e.g. custom_generated_t_style_source:0')

tf.compat.v1.flags.mark_flags_as_required(['model_path', 'output_tensor_name'])
  ...
  def __init__(self):
    ...
    with self.sess.as_default():
      ...
      if FLAGS.input_tensor_name:
        ...
        self.images_placeholder =  tf.Variable(tf.ones(dtype=tf.uint8, shape=[None, None, None]))
...
if __name__ == '__main__':
  tf.compat.v1.flags.DEFINE_string('input_image_path',"./demo/inference_input/",
                         'Path to the input image (or directory containing images) used for inference.'
                         'Used for image translation model e.g. TwinGAN')
  tf.compat.v1.flags.DEFINE_integer('num_output', None,
                          'Number of outputs. Used for generative model, e.g. PGGAN or other kinds of GANs.')
  tf.compat.v1.flags.DEFINE_string('output_image_path', "./demo/inference_output/",
                         'Path to output the output image (or directory).')
  tf.compat.v1.flags.mark_flags_as_required(['output_image_path'])
